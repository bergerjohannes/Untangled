export const SimpleLogo = ({ theme = Theme.Light }: { theme?: Theme }) => (
  <Logo showText={false} theme={theme} />
)

export const LogoWithText = ({ theme = Theme.Dark }: { theme?: Theme }) => (
  <Logo showText={true} theme={theme} />
)
export enum Theme {
  Dark = 'dark',
  Light = 'light',
...
const Logo = ({ showText, theme }: LogoProps) => (
  <div className='flex space-x-4 items-center'>
    <svg className='w-12 h-12' viewBox='0 0 62 62' fill='none' xmlns='http://www.w3.org/2000/svg'>
      <path
        ...
        stroke={theme === Theme.Light ? '#F5F5F5' : '#121418'}
        strokeWidth='2'
        strokeLinecap='round'
    ...
    {showText && (
      <h1
        className={`text-3xl font-bold ${theme === Theme.Dark ? 'text-blackish' : 'text-whitish'}`}
      >
        UNTANGLED NOTES
      </h1>
    )}
